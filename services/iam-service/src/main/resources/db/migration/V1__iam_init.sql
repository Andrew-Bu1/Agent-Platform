...
CREATE TABLE IF NOT EXISTS roles (
    ...
    -- Unique key per tenant. NULL tenant_id (platform roles) is handled by the partial
    -- index below because a UNIQUE constraint treats each NULL as distinct in Postgres.
    CONSTRAINT uq_roles_tenant_key
        UNIQUE (tenant_id, key)
);

-- Enforce unique key for platform-level roles (tenant_id IS NULL).
CREATE UNIQUE INDEX IF NOT EXISTS uq_roles_platform_key ON roles(key) WHERE tenant_id IS NULL;
...
-- Only one row may carry status = 'active' at any time.
-- Partial unique index: NULLs and non-active rows are excluded, so retired/revoked
-- keys are not affected.  This is a second line of defence after the advisory lock.
CREATE UNIQUE INDEX IF NOT EXISTS uq_oauth_signing_keys_one_active
    ON oauth_signing_keys (status)
    WHERE status = 'active';
