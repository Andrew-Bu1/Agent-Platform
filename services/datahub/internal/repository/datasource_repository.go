...
func (r *DatasourceRepository) Insert(ctx context.Context, d *model.Datasource) error {
	const q = `
		INSERT INTO datasources (id, tenant_id, workspace_id, name, description, status, created_by_user_id, created_at, updated_at)
...
func (r *DatasourceRepository) GetByID(ctx context.Context, id, tenantID, workspaceID uuid.UUID) (*model.Datasource, error) {
	const q = `
		SELECT id, tenant_id, workspace_id, name, description, status, created_by_user_id, created_at, updated_at
		FROM datasources
